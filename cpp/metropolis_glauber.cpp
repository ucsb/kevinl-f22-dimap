...
int Metropolis_Glauber_Complete::run(float beta) {
    ...
    while (grids[0] != grids[1]) {
        ...
        flip(grids[0], beta_scaled, point, color, rand);
        flip(grids[1], beta_scaled, point, color, rand);
        steps += 1;
    }
...
void Metropolis_Glauber_Complete::flip(Grid& g, float beta, int index, unsigned char new_color, float rand) {
    unsigned char old_color = g.graph[index];
    float prob_accept;
