...
if len(sys.argv) != 4:
    sys.exit("Usage: grid_mixing.py grid_dim step_size max_iters")
...
bvals, bsteps = simulate(grid.Magnetization(grid.heat_bath_flip, shape, max_iters), step_size)
plt.plot(bvals, bsteps, color='g', label="Heat Bath (Magnetization)")
bvals, bsteps = simulate(grid.Magnetization(grid.metropolis_flip, shape, max_iters), step_size)
...
plt.title("Ising model mixing metrics for {:d}x{:d} grid".format(dim, dim))
plt.axvline(x = b_crit, color = 'r', linestyle = '-', label="Critical β")
plt.legend(loc="upper left")
plt.savefig("grid_mixing{:d}x{:d}.png".format(dim, dim))
